...
for key in $(printf "%s\n" "${!SERVICES[@]}" | sort); do
  ...
  if [ -n "${!vport_forward}" ] && [ -n "${!vport_expose}" ]; then
    ...
    while true; do
      next_vport_forward="${vport_forward}${next_port}"
      next_vport_expose="${vport_expose}${next_port}"

      if [ -n "${!next_vport_forward}" ] && [ -n "${!next_vport_expose}" ]; then
        create_script="${create_script} --publish ${!next_vport_forward}:${!next_vport_expose}"
